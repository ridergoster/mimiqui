...
class Steganography:
    ...
    def __init__(self,  url, compression=4):
        ...
        self.compression = compression if (8 >= compression > 0) else 4
    ...
    def encode(self, data):
        ...
        data = [ord(c) for c in str(lengthData)] + [ord('@')] + data  # add a character to delimit the end of the message
        data = ''.join([format(char, 'b').zfill(8) for char in data])  # fill the last bits of the message to be valid
        ...
        for x in range(self.height):
            for y in range(self.width):
                if index + (3 * self.compression) <= lengthBits:
                    pixel = self.image.getpixel((y, x))  # we get each color's byte of the pixel

                    # And we hide the message progressively
                    ...
                    if self.image.mode == 'RGBA':
                        encoded.putpixel((y, x), (r, g, b, pixel[3]))  # do not forget to add the alpha
                    else:
    ...
    def decode(self):
        ...
        for x in range(self.height):
            for y in range(self.width):
                # For each byte of color
                for color in self.image.getpixel((y, x))[:3]:
                    for i in range(self.compression)[::-1]:
                        buffer += ((color & (2 ** i)) >> i) << (8 - 1 - counter)  # we retrieve each bit one by one
                        counter += 1
                        if counter == 8:
                            data.append(buffer)
                            buffer, counter = 0, 0
                            # Check if we reach the end of the message thanks to the delimiter character
